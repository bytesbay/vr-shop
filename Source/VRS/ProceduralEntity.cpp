	...
	: _requiresFullRecreation(true), _meshCurrentlyProcessed(0)
{
	...
	if (_mesh) {
		...
		_mesh->SetCollisionProfileName(TEXT("Furniture"));
...
void AProceduralEntity::processNode(aiNode* node, const aiScene* scene) {
	UE_LOG(LogTemp, Warning, TEXT("Processing a node (%i childs)"), node->mNumChildren);
...
void AProceduralEntity::loadModel(std::string model) {
	Assimp::Importer importer;

	UE_LOG(LogTemp, Warning, TEXT("START LOGGING"));
	const aiScene* scene = importer.ReadFile(model, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_GenNormals);

	if (!scene) {
		FFileHelper::SaveStringToFile(importer.GetErrorString(), *(FPaths::GameDir() + "loller.log"));
		return;
	}
	...
	// UE_LOG(LogTemp, Warning, TEXT("START LOGGING %i"), scene->mNumMaterials);
	// aiString path;
	// scene->mMaterials[0]->GetTexture(aiTextureType_DIFFUSE, 0, &path);
	//
	// FFileHelper::SaveStringToFile(path.C_Str(), *(FPaths::GameDir() + "loller.log"));
}

// void AProceduralEntity::loadModelFromMemory(const void * buffer, uint32_t buffer_size) {
// 	Assimp::Importer importer;
//
// 	UE_LOG(LogTemp, Warning, TEXT(" -- START LOGGING, BUFFER SIZE: %i"), buffer_size);
//
// 	const aiScene* scene = importer.ReadFileFromMemory(buffer, buffer_size, aiProcess_Triangulate | aiProcess_FlipUVs | aiProcess_GenNormals);
//
// 	if (!scene) {
// 		FFileHelper::SaveStringToFile(importer.GetErrorString(), *(FPaths::GameDir() + "assimp-err.log"));
// 		return;
// 	}
//
// 	_meshCurrentlyProcessed = 0;
